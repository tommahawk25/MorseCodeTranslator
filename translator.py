...
class MorseCodeTranslator:
    ...
    def letter_translation(self, temp_result: str) -> str:
        self.letter = alphabet.get(self.temp_result)
    ...
    def text_or_morse(self, message: str) -> bool:
        dots = self.message.count(".")
        dashes = self.message.count("-")
        slashes = self.message.count("/")
        if dots + dashes + slashes >= len(self.message) / 2:
            is_text = False
            self.translate_to_text(self.message)
        else:
            is_text = True
            self.translate_to_morse(self.message)
        return is_text
    ...
    def translate_to_text(self, message: str) -> str:
        try:
            count: int = 0
            spaces: int = self.message.count(" ")
            double_slashes: int = self.message.count("//")
            ...
            if spaces > 0 and double_slashes == 0:
                for symbol in self.message:
                    ...
                    elif symbol == " " and self.message[count] == "/" and self.message[count+1] == " ":
                    ...
                    elif symbol == "/" or (symbol == " " and self.message[count-2] == "/"):
                        pass
            ...
            # Morse code format 2 (spaces between letters are "/" and spaces between words are "//")
                ...
                for symbol in self.message:
                    ...
                    elif count == len(self.message) - 1 and (
                            self.message[len(self.message) - 2] == "/" or self.message[len(self.message) - 2] == "|"):
                        pass
                    elif count == len(self.message) and (
                            self.message[len(self.message) - 1] == "/" or self.message[len(self.message) - 1] == "|"):
                        pass
                    ...
                    # We have to find out if it is space between words or letters:
                        ...
                        if self.message[count - 2] == "/" or self.message[count - 2] == "|":
                            self.translation += " "
